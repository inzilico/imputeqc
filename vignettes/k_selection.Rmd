...
Assume you have a Plink files and dicided to impute the missing genotypes with fastPHASE tool. The number of haplotype clusters (K) is required as an argument. One option is to leave the default value of 15. For the most cases it will probably work. But what if you have several populations in one file? Say, you search for the fingerprints of selection with hapFLK tool which utilizes the fastPHASE algorithm.

How many clusters to choose for 4, 6, 8 or even more populations in a pool? Tests revealed that K influences the results of hapFLK output. If fastPHASE had an option to estimate the error of imputation, the life would be easire. We could apply the masked analysis. 
...
1. Convert Plink files to fastPHASE \*.inp files, using Plink tool. Check that alleles are coded as letters or numbers. The missing ones should be `?`. 

If your data look differently, send me a chunk of them. I'll figure out what to do next.   
...
Assume you have `chr1.{ped, map}` files in the current directory. Run from the command line
...
2. Get the path to `make_test_files.R` which comes with `imputeqc`. Start R and enter 
