...
Assume you have a Plink files and you dicided to impute the missing genotypes with fastPHASE tool. The number of haplotype clusters (K) is required. One option is to leave the default value of 15. For most cases it will probably work. But what if you have several population in one file because you want to find out the fingerprints of selection with hapFLK tool that utilizes the fastPHASE algorithm?

How many clusters to choose for 4, 6, 8 or even more populations in a pool? A quick tests revealed that K influences the results of hapFLK output. If fastPHASE had an option to estimate the error of imputation, the life would be easire. We could apply the masked analysis. 

The idea of the analysis is quite simple. Mask some genotypes, impute them, then count the error of imputation. Repeat the trick several times with different K and choose the parameter with minimum error of imputation.      

Below is the implementation of the masked analysis on the base of `imputeqc` R package. Let's start from a list of 4 main steps.
...
If your data look differently, send me a chunk of them. I'll figure what to do next.   

2. Generate a few test files with an R script `make_test_files.R`. It is enclosed to the package. I think five files are enough to start. 

3. Impute the missing genotypes in each test file with fastPHASE. Apply different K for each set of files. 

This step takes a lot of computational time depending on the number of populations under consideration, the number markers, and, of course, the K.    
...
Assume you have in the current directory `chr22.{ped, map}` files. Run in command line

    plink --file chr22 --recode fastphase-1chr --out chr22
    
It will create `chr22.recode.phase.inp` ready for further further manipulations. 

The usage of make\_test\_files.R
---------------------------------

1. Install `imputeqc` R package from GitHub. Run in R
...
2. Get path to `make_test_files.R` that comes with `imputeqc` 
...
4. hapFLK software. [link](https://forge-dga.jouy.inra.fr/projects/hapflk) 

5. hapFLK tutorial. [link](https://github.com/bcm-uga/SSMPG2017/blob/master/Presentations/hapflk/hapflk.org)

5. Plink tool. [link](http://zzz.bwh.harvard.edu/plink/)

6. Chan AW, Hamblin MT, Jannink J-L. Evaluating Imputation Algorithms for Low-Depth Genotyping-By-Sequencing (GBS) Data. Feltus FA, ed. PLoS ONE. 2016;11(8):e0160733. [PubMed](https://www.ncbi.nlm.nih.gov/pubmed/27537694)
